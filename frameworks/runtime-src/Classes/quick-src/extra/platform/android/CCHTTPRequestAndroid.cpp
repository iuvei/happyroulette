...
{
	CCLOG("initWithUrl method = %d",method);
	...
	CCLOG("m_httpMethod = %s,",m_httpMethod);
    ++s_id;
...
void HTTPRequest::onRequest(void)
{
    ...
    m_errorCode = code;
    m_responseCode = code;
    m_errorMessage = (code == 200 || code == 201) ? "" : getResponedErrJava();
    // m_state = (code == 200 || code == 201) ? kCCHTTPRequestStateCompleted : kCCHTTPRequestStateFailed;
